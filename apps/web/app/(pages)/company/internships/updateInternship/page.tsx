...
// Helper to normalize work mode from DB
...
export default function EditInternshipPage() {
  ...
  const [formData, setFormData] = useState<InternshipFormData | null>(null);
    ...
    if (userStr) {
      try {
        ...
        if (user.id) {
          ...
          setErrorMessage("No company ID found. Please log in again.");
          router.push("/login");
        }
      } catch {
  ...
  useEffect(() => {
    ...
    const fetchInternship = async () => {
      ...
      const result = await getCompanyInternshipById(internshipId, companyId);
      ...
      if (result.success && result.data) {
        ...
        setErrorMessage(result.message || "Failed to load internship");
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateForm() || !internshipId || !formData) {
      return;
    }
    ...
    const result = await updateCompanyInternship({
      id: internshipId,
    ...
    if (result.success) {
      ...
      setTimeout(() => {
        router.push(`/company/internships/${internshipId}`);
      }, 1500);
    } else {
      setErrorMessage(result.message || "Failed to update internship");
    }
  };
      ...
      <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[14px] text-[#6A7287] mb-4">
            The internship you&apos;re trying to edit doesn&apos;t exist or you don&apos;t have permission.
          </p>
          <button
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      {/* Error Toast */}
      {errorMessage && !isLoading && formData && (
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          exit={{ opacity: 0, y: -20 }}
          ...
          >
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                />
                {errors.title && (
                  <p className="mt-1 text-[12px] text-[#E82646]">{errors.title}</p>
                )}
              </div>
              ...
              <div>
                ...
                />
                {errors.description && (
                  <p className="mt-1 text-[12px] text-[#E82646]">{errors.description}</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                  <label className="block text-[14px] font-medium text-[#202C4B] mb-2">
                    Field <span className="text-[#E82646]">*</span>
                  </label>
                  <select
                    name="field"
                  ...
                  {errors.field && (
                    <p className="mt-1 text-[12px] text-[#E82646]">{errors.field}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {errors.duration && (
                    <p className="mt-1 text-[12px] text-[#E82646]">{errors.duration}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-5">
                ...
                <div className="flex flex-wrap gap-3">
                  {(["on_site", "remote", "hybrid"] as const).map((mode) => (
                    <button
                      ...
                      onClick={() =>
                        setFormData((prev) =>
                          prev ? { ...prev, workMode: mode } : null
                        )
                      }
                      className={`px-4 py-2.5 rounded-lg text-[14px] font-medium border transition-all ${
                        formData.workMode === mode
                          ? "bg-[#3D5EE1] text-white border-[#3D5EE1]"
                          : "bg-[#F8FAFC] text-[#515B73] border-[#E9EDF4] hover:border-[#3D5EE1]"
                      }`}
                    >
                      {mode === "on_site"
                        ? "On-site"
              ...
              {formData.workMode !== "remote" && (
                <div>
                  ...
                  />
                  {errors.location && (
                    <p className="mt-1 text-[12px] text-[#E82646]">{errors.location}</p>
                  )}
                </div>
              )}
            </div>
